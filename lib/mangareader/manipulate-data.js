...
async function getUpdates( id, mangaList ) {
  ...
  await mangaList.forEach( async manga => {
    const userChapter = getUserChapter( id, manga.name, manga.provider );
    const latestChapter = await getLastChapterFromLatest( manga.name, manga.provider );

    if ( latestChapter > userChapter ) res.push( { name: manga.name, chapter: userChapter + 1 } );
  } );
